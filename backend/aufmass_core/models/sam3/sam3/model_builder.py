...
def download_ckpt_from_hf():
    ...
    try:
        # Versuche zuerst, nur lokalen Cache zu nutzen (kein Netzwerk-Zugriff!)
        _ = hf_hub_download(repo_id=SAM3_MODEL_ID, filename=SAM3_CFG_NAME, local_files_only=True)
        checkpoint_path = hf_hub_download(repo_id=SAM3_MODEL_ID, filename=SAM3_CKPT_NAME, local_files_only=True)
        return checkpoint_path
    except Exception as e:
        # Falls nicht im Cache, lade von HuggingFace herunter (nur beim ersten Mal)
        print(f"[SAM3] Model not found in local cache, downloading from HuggingFace...")
