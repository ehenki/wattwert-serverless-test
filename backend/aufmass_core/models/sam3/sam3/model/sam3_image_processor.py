...
class Sam3Processor:
    ...
    def __init__(self, model, resolution=1008, device="cpu", confidence_threshold=0.5):  # Fixed: default device="cpu"
        self.model = model
        ...
        self.find_stage = FindStage(
            img_ids=torch.tensor([0], device=self.device, dtype=torch.long),  # Fixed: use self.device
            text_ids=torch.tensor([0], device=self.device, dtype=torch.long),  # Fixed: use self.device
            input_boxes=None,
