...
export default function UserPoll({ onRating, userId, accessToken, pdfProps, userRole }: UserPollProps) {
  ...
  const handleDownloadClick = async () => {
    ...
    const downloadLink = document.querySelector('#hidden-pdf-download-link a') as HTMLElement;
    if (downloadLink) {
      downloadLink.click();
    }
  };
  ...
  return (
    <div style={{ padding: '20px', textAlign: 'center' }}>
      ...
      <div id="hidden-pdf-download-link" style={{ display: 'none' }}>
        <PDFDownloadLink
          document={<OfferPDF {...pdfProps} />}
          fileName={`${pdfProps.address.replace(/[ ,]/g, '_')}_Schnellaufmass_${new Date().toISOString().split('T')[0]}.pdf`}
        >
          Download
        </PDFDownloadLink>
      </div>
